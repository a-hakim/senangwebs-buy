...
class SWB {
  ...
  updateCustomProperties() {
    ...
    document.documentElement.style.setProperty(
      "--swb-color-primary-rgb",
      primaryRGB
    );
    document.documentElement.style.setProperty(
      "--swb-color-secondary-rgb",
      secondaryRGB
    );
  }
  ...
  initializeCatalogs() {
    ...
    catalogElements.forEach((catalog) => {
      ...
      this.initializeStore(storeId, {
        ...
        checkoutTitle:
          catalog.getAttribute("data-swb-checkout-title") || "Your Cart",
        billingTitle:
          catalog.getAttribute("data-swb-billing-title") || "Billing Details",
        submitButtonText:
          catalog.getAttribute("data-swb-submit-text") || "Proceed to WhatsApp",
        enableBilling:
          catalog.getAttribute("data-swb-enable-billing") !== "false",
  ...
  initializeIndependentButtons() {
    document
      .querySelectorAll("[data-swb-cart][data-swb-store-id]")
      .forEach((button) => {
        if (!button.hasAttribute("data-swb-catalog")) {
          ...
          const secondaryColor = button.getAttribute(
            "data-swb-color-secondary"
          );
          if (primaryColor) this.colors.primary = primaryColor;
          if (secondaryColor) this.colors.secondary = secondaryColor;
    ...
    document
      .querySelectorAll("[data-swb-product-sku][data-swb-store-id]")
      .forEach((button) => {
        if (!button.hasAttribute("data-swb-catalog")) {
          const storeId = button.getAttribute("data-swb-store-id");
          ...
          if (!store) {
            console.warn(
              `Store ${storeId} not initialized. Make sure you have a cart button with store information.`
            );
            return;
          }
          ...
          button.addEventListener("click", () => {
            const product = {
              ...
              price:
                parseFloat(button.getAttribute("data-swb-product-price")) || 0,
              quantity: 1,
            };
  ...
  createCatalogHeader(catalog, storeId) {
    ...
    searchBox.addEventListener("input", (e) =>
      this.handleSearch(e.target.value, storeId)
    );
  ...
  initializeProduct(productElement, storeId) {
    const buttonsContainer = productElement.querySelector(
      "[data-swb-product-buttons]"
    );
    if (!buttonsContainer) return;

    const externalLink = productElement.getAttribute("data-swb-product-link");
    const externalLinkTitle = productElement.getAttribute(
      "data-swb-product-link-title"
    );
    ...
    if (store && store.info.cartEnabled) {
      const cartBtnTitle = productElement.getAttribute(
        "data-swb-product-add-cart-title"
      );
      ...
      addToCartBtn.onclick = () => {
        const product = {
          ...
          price: parseFloat(
            productElement.getAttribute("data-swb-product-price")
          ),
          quantity: 1,
        };
        this.addToCart(storeId, product);
  ...
  updateProductDisplay(storeId) {
    ...
    const catalog = document.querySelector(
      `[data-swb-catalog][data-swb-store-id="${storeId}"]`
    );
    if (!catalog) return;
  ...
  setupEventListeners() {
    document.addEventListener("click", (e) => {
      if (
        e.target.matches(".swb-modal-close") ||
        e.target.closest(".swb-modal-close")
      ) {
  ...
  updateCartCount(storeId) {
    ...
    document
      .querySelectorAll(
        `[data-swb-store-id="${storeId}"] [data-swb-cart-count]`
      )
      .forEach((counter) => {
    ...
    const modalCounters = document.querySelectorAll(
      `.swb-cart-modal[data-swb-store-id="${storeId}"] .swb-cart-count`
    );
  ...
  renderCart(storeId) {
    const cartModal = document.querySelector(
      `.swb-cart-modal[data-swb-store-id="${storeId}"]`
    );
    if (!cartModal) return;
      ...
      itemElement.innerHTML = `
        ...
        item.sku
                ...
                <div class="swb-item-price">${
                  item.price ? this.formatPrice(item.price * item.quantity) : ""
                }</div>
  ...
  showCheckout(storeId) {
    ...
    if (!modal) {
      ...
      const customFieldsHtml = store.info.customFields
        .map(
          (field) => `
      ...
      modal.innerHTML = `
                <div class="swb-modal-content">
                    ...
                    <div class="swb-cart-total">Total: ${this.formatPrice(
                      0
                    )}</div>
  ...
  formatWhatsAppMessage(storeId, customerInfo) {
    ...
    store.cart.forEach((item) => {
      ...
      if (item.price) {
        message += `  Price: ${this.formatPrice(
